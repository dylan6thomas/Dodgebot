...
class DQNLearner:
    def __init__(self, robot_info, map_node, episodes, gamma=0.95, epsilon=1.0, epsilon_min=0.1, decay=0.995):
        ...
        self.change_difficulty_list = []
        self.change_difficulty_times = []
    ...
    def save_checkpoint(self, save_path, extra_info=None):
        checkpoint = {
            ...
            'change_difficulty_times': self.change_difficulty_times,
            'change_difficulty_list': self.change_difficulty_list
        }
    ...
    def load_checkpoint(self, load_path):
        ...
        self.change_difficulty_times = checkpoint['change_difficulty_times']
        self.change_difficulty_list = checkpoint['change_difficulty_list']

        print(f"[Checkpoint] Loaded model and training state from {load_path}")

        plt.plot(self.running_avg_rewards)
        for x, label in zip(self.change_difficulty_list, self.change_difficulty_times):
            plt.axvline(x=x, color='gray', linestyle='--')
            # plt.text(x, 1.02, label, rotation=90, verticalalignment='bottom', horizontalalignment='center', transform=plt.gca().get_xaxis_transform())
...
class DQNNode(Node):
    ...
    def train_loop(self):
        ...
        if len(self.learner.episode_successes) > self.learner.success_window and self.learner.success_window_avg >= INC_DIFFICULTY_THRESH:
            ...
            self.learner.change_difficulty_list.append(self.learner.episode_count)
            self.learner.change_difficulty_times.append(time.time())
...
def main(args=None):
    ...
    dqn_node = DQNNode(robot_info, map_node, start, goal, load_path="checkpoints/model_distance_20.pt", max_distance=18)
